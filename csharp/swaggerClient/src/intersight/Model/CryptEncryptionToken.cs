...
{
    /// <summary>
    /// A cryptographic token used by Intersight services to authenticate themselves with the encryption service, for the purposes of encrypting or decrypting MO properties that contain sensitive data. The lifetime of the token is generally tied to the lifetime of user sessions or device connections.  However, in case of user triggered workflows, the token may be needed even after the user logs out. The encryption token is created by the encryption service when a user or device session is created  (i.e. user logon or device connects to Intersight) or when a user triggers a workflow.   Once created, the token is valid for a limited duration (typically 1 hour) and will expire if not renewed. To avoid expiry, a token needs to be renewed. The renewal is done by the encryption service for users, devices  and workflows based on notifications from the respective Intersight services, such as the user authentication service, the device connection gateway and the workflow service. When a user logs out or a user session times out (i.e. session is deleted), the token associated with  the user session is also deleted. Similarly, when a device disconnects from Intersight, the  token is deleted. However, any token created due to user triggering a workflow stays as long as workflow completes. Services that need to encrypt or decrypt data invoke the encryption service APIs with the encryption token along with data (like password). When a user saves a secure property (e.g. password), the service owning this MO (and property) invokes the encryption service with the token (passed through security context) &amp; (cleartext)  password it received through the security context. The encryption service uses Hashicorp Vault&#39;s transit  backend to encrypt/decrypt property using a Key derived from Account Key (each account has a Key)  and Master Key (Intersight). The ciphertext returned by encryption service is persisted to database. When workflows are scheduled to run, the requests are sent to the service owning the MO, which invokes the encryption service with the token (passed through security context) and  encrypted password it read from the database. The encryption service decrypt it with a key derived from Account Key (each account has a Key) and Master Key (Intersight). The cleartext data is  returned which is used by the service to provision the devices. If workflows are scheduled in  the future or take a long time to complete, The encryption service will renew the token as long as the authentication service, device connection gateway or workflow service notify that the token needs to be kept alive. 
    /// </summary>
    [DataContract]
    public partial class CryptEncryptionToken :  IEquatable<CryptEncryptionToken>, IValidatableObject
    {
        ...
        /// <param name="AccountKey">A collection of references to the [crypt.AccountKey](mo://crypt.AccountKey) Managed Object.  When this managed object is deleted, the referenced [crypt.AccountKey](mo://crypt.AccountKey) MO unsets its reference to this deleted MO. .</param>
        ...
        /// </summary>
        /// <value>A collection of references to the [crypt.AccountKey](mo://crypt.AccountKey) Managed Object.  When this managed object is deleted, the referenced [crypt.AccountKey](mo://crypt.AccountKey) MO unsets its reference to this deleted MO. </value>
